  ...
  <div class="min-h-screen bg-paper">
    ...
    <div v-else-if="error" class="max-w-7xl mx-auto px-4 py-8">
      <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-sm p-6">
        <h1 class="text-xl font-bold text-red-900 dark:text-red-300 mb-2">Error Loading Event</h1>
        ...
        <div class="flex items-start gap-6">
          ...
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-2">
              ...
              <span class="px-3 py-1 bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-300 text-sm font-medium rounded-full">
                Event
              </span>
              <span
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div v-if="data.summary.summary" class="bg-panel rounded-sm  border border-rule p-6">
            <h2 class="text-xl font-bold text-ink mb-4">Event Overview</h2>
        ...
        <div class="space-y-6">
          ...
          <ClientOnly>
            <WikidataInfoCard :entity-id="data.entity.id" entity-type="event" />
          </ClientOnly>

          <!-- Related Entities -->
          <div v-if="data.relatedEntities.length > 0" class="bg-panel rounded-sm  border border-rule p-6">
            <h2 class="text-lg font-bold text-ink mb-4">Key Figures & Locations</h2>
            <div class="space-y-3">
              <div
...
// Fetch entity data
const { data, pending, error } = await useFetch(`/api/entities/event/${slug}`)

// SEO
const siteUrl = 'https://newsar.codejungle.org'
const pageUrl = `${siteUrl}/event/${slug}`
const eventName = data.value?.entity.name || 'Event'
const pageTitle = `${eventName} - Event - Newsar`
const pageDescription = data.value?.summary.shortDescription
  || data.value?.summary.summary?.slice(0, 160)
  || `News coverage and analysis of ${eventName}`
